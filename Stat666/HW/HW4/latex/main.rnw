...
\section{PSA}
\subsection{Is there a significant relationship between the chemical
...
<<results=tex,echo=F>>=
  print(xtable(B,digits=4,caption="$\\hat{\\bm B}$"),
        size="\\tiny",caption.pla="top")
@

\subsection{What is the essential dimensionality of the relationship
...
\subsection{Use canonical correlation analysis to identify and interpret
            the important dimensions of the relationship between the 
            xs and the ys.}

\subsection{ Traditionally, experts have assumed that heavy metals are
